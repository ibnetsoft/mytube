...
out=root/'v5';out.mkdir(exist_ok=True)
...
html='''<!doctype html><html lang="ko"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>낡은 검보 · 13가지 컷 테스트</title><style>body{margin:0;background:#12191c;color:#eee;font:16px system-ui}main{max-width:1280px;margin:auto;padding:24px}h1{font-size:28px}p{line-height:1.7;color:#b6c5c8}video{width:100%;background:black;border-radius:12px}nav{display:flex;flex-wrap:wrap;gap:8px;margin:20px 0}button{background:#273b40;color:#fff;border:1px solid #486267;border-radius:8px;padding:12px;cursor:pointer}button:hover{background:#43636b}.gallery{display:grid;grid-template-columns:repeat(auto-fit,minmax(340px,1fr));gap:20px}a{color:#bde3dc;text-decoration:none}img{width:100%;border-radius:8px}span{display:block;padding:8px}</style><main><h1>낡은 검보 · 얼굴·소품 보호 배치</h1><p>12가지 방식 + 격자 2×2 / 2×3 비교 · 33컷. 기존 이미지 10장과 기존 대사·음성을 재사용했습니다.<br>얼굴·검보·서찰·검의 보호 영역을 직접 지정해 자막 겹침을 피했습니다. 여백이 부족한 컷은 별도 자막 공간을 확보했습니다. 자동 얼굴 인식 기능이 아닌 이번 샘플의 배치 테스트입니다.</p><video id="player" controls preload="metadata" poster="page_01.png" src="output.mp4"></video><nav>BUTTONS</nav><p>버튼으로 해당 페이지부터 재생합니다. 반복 장면은 컷 구성 비교를 위한 재편집입니다. 연속 동작은 기존 그림의 세부 구도를 순서대로 보여주는 방식이며 실제 인물 애니메이션은 아닙니다.</p><div class="gallery">CARDS</div><p><a href="output.mp4" download>영상 다운로드</a> · <a href="manifest.json">구성·원본 대응표</a></p></main><script>function seek(t){const v=document.getElementById('player');v.currentTime=t;v.play()}</script></html>'''.replace('BUTTONS',buttons).replace('CARDS',cards)
