...
def test_final_story_saved_separately_and_reused_until_text_changes():
    ...
    plan = plan_package_bgm(runner, 'test', package)
    ...
    plan_package_bgm(runner, 'test', package)
    assert runner.calls == 1
    package['script'] += ' 마침내 안도했다.'
    assert plan_package_bgm(runner, 'test', package)['script_version'] != plan['script_version']
