-- =============================================================
-- Migration: Emergency Referral RPC Hotfix (AIR-0166A)
-- Description: Fixes critical schema mismatch where legacy RPCs
-- incorrectly queried 'user_id' instead of 'beneficiary_user_id' 
-- in the commissions table.
-- =============================================================

-- 1. Fix Index
DROP INDEX IF EXISTS public.idx_commissions_user_status_created;
CREATE INDEX IF NOT EXISTS idx_commissions_beneficiary_status_created ON public.commissions(beneficiary_user_id, status, created_at);

-- 2. Fix get_referral_dashboard_kpi
...
BEGIN
    ...
    -- User's Commission KPIs (FIXED: beneficiary_user_id)
    SELECT 
    ...
    -- User's Withdrawal KPIs (UNCHANGED: withdrawal_requests.user_id)
    SELECT 
...
BEGIN
    ...
    tree_with_stats AS (
        SELECT 
            ...
            -- (FIXED: beneficiary_user_id for commissions)
            (SELECT COUNT(*) FROM public.commissions WHERE beneficiary_user_id = d.id AND created_at >= CURRENT_DATE::timestamp) as today_jobs,
            (SELECT COUNT(*) FROM public.commissions WHERE beneficiary_user_id = d.id AND created_at >= date_trunc('month', CURRENT_DATE)::timestamp) as month_jobs,
            (SELECT COALESCE(SUM(amount), 0) FROM public.commissions WHERE beneficiary_user_id = d.id AND created_at >= CURRENT_DATE::timestamp) as today_commission,
            (SELECT COALESCE(SUM(amount), 0) FROM public.commissions WHERE beneficiary_user_id = d.id AND created_at >= date_trunc('month', CURRENT_DATE)::timestamp) as month_commission,
            (SELECT COALESCE(SUM(amount), 0) FROM public.commissions WHERE beneficiary_user_id = d.id AND status = 'APPROVED') as total_commission,
            
            -- Simplified withdrawable balance for tree nodes
            ((SELECT COALESCE(SUM(amount), 0) FROM public.commissions WHERE beneficiary_user_id = d.id AND status = 'APPROVED') - 
