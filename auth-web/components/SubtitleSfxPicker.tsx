...
export default function SubtitleSfxPicker({ locale = 'ko', assets, value, projectId, headers, disabled, onChange, onOpen }: {
    ...
    const play = async (asset: any) => {
        ...
        try {
            ...
            if (!url) {
                ...
                if (!blob.size) throw new Error(ui("효과음 파일이 비어 있습니다."))
    ...
    return <>
        <button type="button" disabled={disabled} onClick={() => {
            ...
            <span className="shrink-0 font-bold">{ui("효과음 선택")}</span>
        ...
        {open && createPortal(<div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/75 p-4" onClick={() => setOpen(false)}>
            <div ref={dialog} role="dialog" aria-modal="true" aria-labelledby="sfx-picker-title" onClick={event => event.stopPropagation()}
                ...
                <div className="grid min-h-0 grid-cols-1 gap-3 overflow-y-auto sm:grid-cols-2">
                    ...
                    {!assets.length && <p className="py-5 text-sm text-gray-400 sm:col-span-2">{ui("등록된 효과음이 없습니다.")}</p>}
                    {!!assets.length && !assets.some(matchesSearch) && <p className="py-5 text-sm text-gray-400 sm:col-span-2">{ui("검색 결과가 없습니다.")}</p>}
                </div>
                <div className="shrink-0 space-y-2 rounded-xl bg-black/20 p-3">
                    <p className="truncate text-xs text-gray-400">{sample || ui("미리듣기 버튼을 눌러 효과음을 확인하세요.")}</p>
                    <audio ref={player} controls className="h-9 w-full" onError={() => setError(ui("효과음 재생에 실패했습니다. 다른 파일을 선택해 주세요."))} />
