...
export default function VoiceStudioPicker({value, direction, onChange, headers, microphone = false, label, description, buttonText}: {
    ...
    return <>
        <button type="button" aria-label={label} title={label} onClick={(event)=>{event.stopPropagation();setDraft(value);setTone(direction);setOpen(true)}} className={microphone ? `${buttonText ? 'px-2.5 gap-1.5 text-[10px] font-bold' : 'w-[30px] sm:w-8'} h-[30px] sm:h-8 rounded-md border border-white/10 bg-[#10141b] text-cyan-100 flex items-center justify-center hover:border-cyan-400/50` : 'w-full max-w-full sm:w-auto sm:max-w-52 truncate px-3 py-1.5 rounded-md border border-cyan-500/40 bg-cyan-500/10 text-cyan-100 text-xs font-bold hover:bg-cyan-500/20'}>{microphone ? <><Mic size={14}/>{buttonText && <span>{buttonText}</span>}</> : <>{buttonText || value.replace('gemini:','')}</>}</button>
        {open&&createPortal(<div onClick={event=>event.stopPropagation()} className="fixed inset-0 z-[100] bg-black/70 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label="Voice Studio 내레이션 목소리">
            <div className="bg-[#1c2027] text-gray-100 [color-scheme:dark] border border-white/20 rounded-xl w-full max-w-3xl max-h-[85vh] flex flex-col p-5 gap-3">
                <div className="flex justify-between"><h2 className="font-bold text-white">Google · 내레이션 목소리 30개</h2><button type="button" onClick={()=>{player.current?.pause();setOpen(false)}} aria-label="닫기">✕</button></div>
