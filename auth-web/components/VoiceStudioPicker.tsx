...
export default function VoiceStudioPicker({value, direction, onChange, headers, microphone = false, label, description}: {
    ...
    return <>
        ...
        {open&&createPortal(<div onClick={event=>event.stopPropagation()} className="fixed inset-0 z-[100] bg-black/70 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label="Voice Studio 내레이션 목소리">
            <div className="bg-[#1c2027] border border-white/20 rounded-xl w-full max-w-3xl max-h-[85vh] flex flex-col p-5 gap-3">
                ...
                <div className="flex gap-2"><input aria-label="목소리 검색" value={search} onChange={e=>setSearch(e.target.value)} placeholder="목소리 이름 검색" className="bg-black/30 rounded p-2 flex-1"/><select aria-label="성별" value={gender} onChange={e=>setGender(e.target.value)} className="bg-[#1c2027] rounded p-2"><option value="">전체</option><option>남성</option><option>여성</option></select></div>
                ...
                <label className="text-xs">말투·감정<input value={tone} maxLength={500} onChange={e=>setTone(e.target.value)} className="w-full bg-black/30 rounded p-2 mt-1" placeholder="예: 담담하고 따뜻하게, 긴장감을 서서히 높이며"/></label>
